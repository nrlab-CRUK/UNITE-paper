...
parser.add_argument('--input_data_path', type=str,
                    default='/scratchc/nrlab/wang04/ulyses_results_iteration/iteration_3_merge_below_3/cnn_model/cnn10/plasma_tp1_0.1x_model_C2/', help='Path to input data')
parser.add_argument('--tf_label', type=str,
                    default='0_0.03', help='TF label')
parser.add_argument('--model_name', type=str,
                    default='tf_efficientnetv2_s.in1k', help='Model name')
parser.add_argument('--remove_problem_bin',
                    action='store_true', help='Remove problem bin')
# parser.add_argument('--output_path', type=str,
# default='/scratchc/nrlab/wang04/ulyses_results_iteration/iteration_3_merge_below_3/cnn_model/cnn7/plasma_tp1_0.1x_model_C2/', help='Path to output files')
...
roc_curve_csv_path_ind_test = roc_curve_csv_path + \
    'ind_test_' + hyperparameter_search_strategy + '/' + model_name + '/'
...
# stop if the first dimension of x_indtest, y_indtest, z_indtest, bam_id_indtest are not the same
if x_indtest.shape[0] != y_indtest.shape[0] or x_indtest.shape[0] != z_indtest.shape[0] or x_indtest.shape[0] != bam_id_indtest.shape[0]:
    raise ValueError(
        'The first dimension of x_indtest, y_indtest, z_indtest, bam_id_indtest are not the same')


# for x and x_indtest, if there are only 3 dimensions, add a new dimension as forth dimension
if len(x.shape) == 3:
    x = x[:, :, :, np.newaxis]
if len(x_indtest.shape) == 3:
    x_indtest = x_indtest[:, :, :, np.newaxis]
...
# read this: https://towardsdatascience.com/getting-started-with-pytorch-image-models-timm-a-practitioners-guide-4e77b4bf9055


# model = timm.create_model(model_name=model_name,
#                           pretrained=True, in_chans=n_input_channel, num_classes=0)
...
# calcualte the sample_weight for x
x_train_sample_weight = class_weight.compute_sample_weight(
    class_weight='balanced', y=y_train)
# convert sample_weight to tensor
x_train_sample_weight = torch.Tensor(x_train_sample_weight)


x_val_sample_weight = class_weight.compute_sample_weight(
    class_weight='balanced', y=y_val)
# convert sample_weight to tensor
x_val_sample_weight = torch.Tensor(x_val_sample_weight)


# set sample weight
x_train = torch.Tensor(x_train.swapaxes(1, 3).swapaxes(2, 3))
# x_train = {'data': torch.Tensor(x_train.swapaxes(1, 3).swapaxes(2, 3))}
# # add sample_weight to the X dict
# x_train['sample_weight'] = x_train_sample_weight
x_val = torch.Tensor(x_val.swapaxes(1, 3).swapaxes(2, 3))
# x_val = {'data': torch.Tensor(x_val.swapaxes(1, 3).swapaxes(2, 3))}
# # add sample_weight to the X dict
# x_val['sample_weight'] = x_val_sample_weight
...
#         nn.Linear(num_in_features, out_features=512, bias=False),
#         nn.ReLU(),
#         nn.BatchNorm1d(512),
#         nn.Dropout(0.2),
...
class MyNet(NeuralNetBinaryClassifier):
    ...
    def get_loss(self, y_pred, y_true, X, *args, **kwargs):
        ...
        sample_weight = skorch.utils.to_tensor(
            X['sample_weight'], device=self.device)
...
if train_from_scratch:
    net = NeuralNetBinaryClassifier(
        ...
        callbacks=[
            ...
            EpochScoring("roc_auc", lower_is_better=False, name='val_auroc'),
        ...
        # validation data
        train_split=predefined_split(Dataset(x_val, torch.Tensor(y_val)))
        # train_split=False
    )
    ...
    if hyperparameter_search_strategy == 'grid':
        ...
        grid_search = RandomizedSearchCV(
            estimator=net, param_distributions=params, n_iter=random_search_iter, cv=inner_cv, scoring=('roc_auc'), verbose=3, random_state=seed)
    grid_search.fit(torch.Tensor(x.swapaxes(1, 3).swapaxes(2, 3)),
                    torch.Tensor(y), groups=z)
    ...
    best_param_wrapped = {k: [v] for k, v in grid_search.best_params_.items()}
...
if not train_from_scratch:
    ...
    net.fit(torch.Tensor(x.swapaxes(1, 3).swapaxes(2, 3)),
            torch.Tensor(y))
...
y_pred = net.predict_proba(torch.Tensor(
    x_indtest.swapaxes(1, 3).swapaxes(2, 3)))[:, 1]
