...
parser <- add_argument(parser, "--theme_file", type = "character", 
                       help = "Theme file.", 
                       default = "/home/nrlab/wang04/ulyses/explore_packaged/functions/themes.r")
...
c_cnv_log2Ratio <- ggplot(bin_df_final) +
  geom_point(aes(bin_index, cnv_log2Ratio), color = "grey", size = 0.3) +
  scale_y_continuous(position = "right", expand = c(0, 0)) +
  scale_x_continuous(expand = c(0, 0)) + 
  labs(y = "Log2Ratio") +
  coord_flip()+
...
c6 <- ggplot(bin_df_final) +
  ...
  scale_x_continuous(breaks = bin_axis_breaks,
                     labels = bin_axis_labels,
                     expand = c(0, 0)) + 
  labs(x = paste("Bin", " (", which_bin_size, ") "), 
...
c7 <- ggplot(col_sd_df_final) +
  geom_line(aes(isize_label, col_sum, group = 1), linewidth = 0.7, color = "grey") +
...
c8 <- ggplot(col_sd_df_final) +
  geom_line(aes(isize_label, col_sd, group = 1), linewidth = 0.7, color = "orange3" ) +
  scale_x_continuous(limits = c(isize_filter_min, isize_filter_max), expand = c(0, 0)) + 
  scale_y_continuous(limits = c(0, 0.000030), expand = c(0, 0), position = "right") +
...
ggsave(filename = ulyses_plot_filename, plot = ulyses_n_isize_plot, width = 10, height = 8 )
