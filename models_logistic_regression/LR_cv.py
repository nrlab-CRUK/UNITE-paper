...
parser = argparse.ArgumentParser(
    description='Logistic Regression Binary Function Nested CV')
...
parser.add_argument('--input_csv', type=str, default="/scratchc/nrlab/wang04/ulyses_results_iteration/iteration_3_merge_below_3/lr_model/lr10/0.1-1_pairwise/xgboost_input.feat.cnv.tf.0.1-1_pairwise.csv", help='Input CSV file path')
...
# check the shape of x, if not a dataframe, change it to a dataframe
    ...
    for fold_i, (train_idx, test_idx) in enumerate(outer_cv.split(x, y, groups=z)):
        ...
        if hyperparameter_search_strategy == 'grid':
            ...
            grid_search = RandomizedSearchCV(
                estimator=model, param_distributions=lr_params, n_iter=random_search_iter, cv=inner_cv, scoring=('roc_auc'), verbose=3, random_state=repeat_seed)
