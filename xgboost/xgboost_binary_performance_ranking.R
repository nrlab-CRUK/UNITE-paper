...
plot_file <- file.path(nested_cv_plot_path, "xgboost_ranking_plot_main_fig.pdf")
plot_file_csv <- file.path(nested_cv_plot_path, "xgboost_ranking_plot_main_fig.csv")
plot_file_indtest <- file.path(indtest_plot_path, "xgboost_ranking_plot_indtest_main_fig.pdf")
indtest_csv <- file.path(indtest_plot_path, "xgboost_indtest_scores.csv")
plot_file_plus_all_feat <- file.path(nested_cv_plot_path, "xgboost_ranking_plot_all_feat_appendix.pdf")
plot_file_plus_all_feat_indtest <- file.path(indtest_plot_path, "xgboost_ranking_plot_all_feat_indtest_appendix.pdf")
...
feat_lv <- c(all_feat_name_new, "Length", "SD", "CNV", "C/T", "S/L")
ans2$feat <- factor(ans2$feat, levels = feat_lv)
xgb_model_label <- c("X6", "X1", "X2", "X3", "X4", "X5")
mainplot_legend_labels <- paste(xgb_model_label, feat_lv, sep = "-")
...
fl_indtest_read_bind_all_feature_model <- fl_indtest_read_bind %>%
  filter(feat == "cnv_sd_length_ctRatio_slRatio")
...
fl_read_bind <- fl_read_bind %>%
  filter(str_detect(feat, "cnv_sd_length_ctRatio_slRatio"))
...
df <- df %>%
  mutate(plot_file = paste0(indtest_plot_path, "/xgboost_pred_score_plot_indtest_tf_", ichorcna_strat_label, ".pdf")) %>%
...
# copy shap from shap_files to /home/nrlab/wang04/ulyses/1_nested_cv_plots/xgboost_SHAP
shap_file1 <- file.path(
  wd,
  "0-0.03_pairwise",
  "xgboost_input.feat.cnv_sd_length_ctRatio_slRatio.tf.0-0.03_pairwise_roc_curve_csv",
...
shap_file2 <- file.path(
  wd,
  "0.03-0.1_pairwise",
  "xgboost_input.feat.cnv_sd_length_ctRatio_slRatio.tf.0.03-0.1_pairwise_roc_curve_csv",
  "SHAP.average_SHAP_summary_plot.dot.pdf"
)
...
shap_file3 <- file.path(
  wd,
  "0.1-1_pairwise",
  "xgboost_input.feat.cnv_sd_length_ctRatio_slRatio.tf.0.1-1_pairwise_roc_curve_csv",
  "SHAP.average_SHAP_summary_plot.dot.pdf"
)
shap_file4 <- file.path(
  wd,
  "all",
  "xgboost_input.feat.cnv_sd_length_ctRatio_slRatio.tf.all_roc_curve_csv",
  "SHAP.average_SHAP_summary_plot.dot.pdf"
)
shap_files <- c(shap_file1, shap_file2, shap_file3, shap_file4)

# # issue warning if any of the shap_files does not exist
if (!all(file.exists(shap_files))) {
  message("Some of the shap_files do not exist")
}
...
# issue warning if any of the shap_files_new does not exist
if (!all(file.exists(shap_files_new))) {
  message("Some of the shap_files_new do not exist")
}

shap_dir <- "/home/nrlab/wang04/ulyses/1_nested_cv_plots/xgboost_SHAP"
