...
parser.add_argument('--input_csv', type=str, default="/scratchc/nrlab/wang04/ulyses_results_iteration/iteration_3_merge_below_3/xgboost_model/xgb6/0.2-1_pairwise/xgboost_input.feat.cnv_sd_length_ctRatio.tf.0.2-1_pairwise.csv", help='Input CSV file path')
...
# get the independent test csv file by substitute 'xgboost_input' with 'xgboost_input_independent_test'
independent_test_csv = input_csv.replace(
    'xgboost_input', 'xgboost_input_independent_test')
...
if hyperparameter_search_strategy == 'grid':
    ...
    grid_search = RandomizedSearchCV(
        estimator=model, param_distributions=xgb_params, n_iter=random_search_iter, cv=inner_cv, scoring=('roc_auc'), verbose=3, random_state=seed)
...
with open(grid_search_pickle_path, 'wb') as f:
    ...
    # grid_search2 = RandomizedSearchCV(model, param_distributions=xgb_params, n_iter=5,
