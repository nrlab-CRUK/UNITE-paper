...
cnn14_xgb10_testid <- read_csv("/scratchc/nrlab/wang04/ulyses_results_iteration/iteration_3_merge_below_3/xgboost_model/xgb10/all/xgboost_input_independent_test.feat.sd.tf.all.csv") |> pull(primary) |> unique()
...
if (TRUE) {
  ...
  final_test <- all_long_filter %>%
          ...
          filter(primary %in% cnn14_xgb10_testid)
