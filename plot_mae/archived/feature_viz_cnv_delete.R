...
plot_saving_dir <- "/home/nrlab/wang04/ulyses/plot_mae/plots/cnv"
...
filter_criteria <-  (!mae$author %in% c('M.S. et al', 'Stephen Cristiano', 'K.S. et al')) & 
                    (mae$timepoint == 1 | is.na(mae$timepoint))
...
all_cnv <- all_long_filter %>%
  dplyr::filter(assay == !!assay_to_keep) %>%
  mutate(y = value) %>%
...
# calculate the trimmed MAD for each sample

all_cnv_grouped <- all_cnv %>%
  select(primary, rowname, y) %>%
  group_by(primary)

all_cnv_list <- group_split(all_cnv_grouped)
# set the names of the list to be the primary
all_cnv_list <- set_names(all_cnv_list, group_keys(all_cnv_grouped))

# plot the MAD for each sample

p_cnv <- ggplot(data = all_cnv_list[[10]], aes(rowname, y)) +
geom_point(color = "black", alpha = 0.5) +
# set y-axis limit -1 to 1
ylim(-1, 1) +
theme_classic() +
# rotate x-axis tick and text by 45 degree
theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
# make the x-axis text smaller
theme(axis.text.x = element_text(size = 6))

ggsave(
  filename = file.path(plot_saving_dir, "cnv_mad.pdf"),
  plot = p_cnv,
  width = 6,
  height = 4,
  dpi = 300
...
message("Saved to ", file.path(plot_saving_dir, "cnv_mad.pdf"))
